...
final class UpdaterViewModel: NSObject, ObservableObject, SPUUpdaterDelegate {
  ...
  func updater(_ updater: SPUUpdater, didAbortWithError error: Error) {
    ...
    NSLog(
      ...
      nsErr.localizedDescription, nsErr.userInfo as NSDictionary)
    if let underlying = nsErr.userInfo[NSUnderlyingErrorKey] as? NSError {
      NSLog(
        "Sparkle: underlyingError domain=%@ code=%ld desc=%@ userInfo=%@", underlying.domain,
        underlying.code, underlying.localizedDescription, underlying.userInfo as NSDictionary)
    }
  }
  ...
  func updater(
    ...
    NSLog(
      "Sparkle: willDownloadUpdate version=%@ url=%@", item.versionString,
